...
const Admin = () => {
  ...
  useEffect(() => {
    if (activeTab === "manage") {
      axios
        .get("https://house-new.netlify.app/.netlify/functions/api/ruben")
        .then((response) => {
  ...
  const handlePostSave = async (imageUrl) => {
    try {
      ...
      if (editingPost) {
        ...
        await axios.put(
          `https://house-new.netlify.app/.netlify/functions/api/ruben/${editingPost._id}`,
          postData
        );
        ...
        // Create new post
        await axios.post(
          "https://house-new.netlify.app/.netlify/functions/api/ruben/create",
          postData
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `https://house-new.netlify.app/.netlify/functions/api/ruben/${id}`
      );
  ...
  return (
    <div
      className="text-slate-800 sidebar"
      style={{ display: "flex", height: "100vh" }}
    >
      <div
        className="text-slate-800 small  "
        style={{
          width: "200px",
          background: "#333",
          color: "#fff",
          ...
          <li style={{ marginBottom: "10px" }}>
            <button
              onClick={() => {
                setActiveTab("create");
                setEditingPost(null);
              }} // Clear editing state when creating a new post
              style={{
                width: "100%",
                padding: "10px",
